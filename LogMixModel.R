...
cat("model FIRST
 {
  ...
  for (i in 1:N) {
    ...
    logit(p[i]) <- b0[group[i]] + b1[group[i]]*time 
  }
  # M groups
  for (j in 1:M) {
    b0[j] ~ dnorm(0,1E+2)
  ...
  dataList = list("N","time","bin_score","Nsubj","group")
  params = c("b0","b1")
  
  inits <- list(b0=rep(0.1,Nsubj),b1=rep(0.1,Nsubj))
  ...
  attach(data)
  bugs.out <- bugs(data=dataList,inits=NULL, parameters.to.save=params, model.file="model.txt", 
